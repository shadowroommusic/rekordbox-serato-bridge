...
def preview(source_name: str, source: list[Track], target_name: str, target: list[Track]) -> dict:
    ...
    for item in source:
        ...
        matches.append(Match(item.source_id, found.source_id if found else None, method, confidence, len(item.cues), item.path, found.path if found else None, tuple(warnings)))
    ...
    return {
        ...
        "target": {"asset_count": len(target), "local_asset_count": len(local_target), "streaming_asset_count": sum(item.source_kind == "streaming" for item in target)},
